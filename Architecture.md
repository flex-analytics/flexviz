...
- 4 downsampling strategies, selected via `downsample` param. `minmax`, `lttb` and `fpcs` share one **x-width bucket pass**; `nth` stands apart.
  ...
  - **`"nth"`** — uniform stride, no grid and no data check, only the dtype gate. No kernel: `_nth_agg_expr` is one Polars `gather` whose positions (`0, s, 2s, ...`, `s = max(1, len // n_points)`, at most `n_points`) come from the column's own `len()`, so the same expression serves a resident `select()` (grouped or not — grouped = row-count buckets) and a grouped `group_by` on either source kind. A scan cannot stream a `gather`, so it materializes the gathered columns (the pruned window when zoomed). Ungrouped on a scan therefore takes `nth_plan`: two streaming passes (`pl.len()` for stride, then `gather_every(stride).head(n_points)`) because one streaming plan cannot count and gather; unfiltered the count is Parquet metadata, and a viewport is a frame-level filter the reader prunes row groups with. `gather_every` keeps stride phase across morsels (undocumented; `TestNthScanPlan` pins it). Grouped on a scan runs the expression inside the fused `group_by`; Polars 1.44 falls back to an in-memory `group_by` for a non-reduction aggregation, so that case is not out-of-core (`test_ooc.py` marks `line-grouped-nth` a strict xfail; `TestGroupedNthScanPlan` pins parity with the resident frame). Every-nth is a stride, not a uniform sample: the tail past `n_points * s` is never read.
