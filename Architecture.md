...
└── process(event, trace_infos, viewports_by_figure, cross_filter_mode) → List[TraceDelta]
      ...
      9. In overlay mode, per partition: an owner partition computes only bg,
         ...
         specs of all partitions run in one aggregate call; fg runs once per
...
- `_programmaticOps` counts the programmatic Plotly operations in progress per figure (`fvRunProgrammaticPlotlyOp(figUid, ...)`: axis locks, drag mode, selection box snap-back, heatmap overlay colorbars). While a figure has one, its handlers ignore its Plotly events; other figures stay interactive. Reset and deselect do not hold it: `Plotly.react` emits no relayout, and a guard held for the round trip would drop the user's zooms. `handleSelected` additionally checks `_selectionMatches` + `_selectionBoxMatches` to suppress the spurious `plotly_selected` re-fire that Plotly emits after a `Plotly.react` re-render.
