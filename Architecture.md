...
- **Targets** (`get_cube_target_spec(axis_range, schema)`; dim order is part of cube identity):
  ...
  delta replaces the approximate envelope — keeping commit ≡ share/restore bit-exact. The two now
  bucket the same way (both are x-bucket envelopes), so they agree up to plateau ties and the f32
  packing of the cube partials.
