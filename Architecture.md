...
- Four downsampling strategies, selected via `downsample` param:
  ...
  - **`"minmax"` (default)** — min-max envelope: splits into `n_points // 2` buckets, keeps the argmin + argmax of y per bucket, and flattens the two into points. Preserves extrema and spikes on every path.
    ...
    - **Scan source.** The kernel would materialise the whole column, so the engine swaps in `_pairs_plan`: one streaming `group_by` collect over the same grid, with `min_by`/`max_by` for the x at each extremum. That plan is order-independent, drops null x and tolerates NaN, so a scan needs only the dtype gate.
