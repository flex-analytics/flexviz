...
- Three downsampling strategies, selected via `downsample` param:
  ...
  - **`"minmax"` (default)** — min-max envelope: splits into `n_points // 2` buckets, selects argmin + argmax of y per bucket and gathers x and y at those indices. An **ungrouped** line buckets by equal **x width**; both source kinds build the same grid (`_bucket_grid`), so they return the same y multiset and differ only in which member of an exact y plateau they pick. The grid spans the viewport when zoomed, or the engine-resolved unfiltered `(min, max)` when not (`domain_cols` requests `x_col` for every unzoomed ungrouped minmax line, on either source kind), so a cross-filter never moves the bucket edges. On a resident frame this is the single `minmax_line` Rust kernel call with an `x_domain` (one kernel call on purpose: Polars does not CSE opaque plugin expressions, so the earlier two-gather form ran the whole scan twice per trace); x-width buckets need a well-formed x, so the engine calls `LFQueryBuilder.check_line_x(x_col)` before resolving the domains — numeric or temporal (read off the schema, no collect), then an O(1) null count, one pass asserting ascending order, and, on a float dtype, an O(1) trailing-NaN probe (NaN sorts last, so on a sorted null-free column every NaN is a suffix; a NaN in the middle already fails the order pass, which then counts them to report the real defect); memoized per source and column, raising `ValueError` otherwise, skipped by `assume_sorted_x=True`. It runs before the domain resolve because the sorted flag it leaves behind makes the min/max collect O(1). `Figure.add_line` applies the same dtype gate at build time when the figure holds the frame, and `n_points` is bounded to `[2, 25000]` in `LinePlot.__init__`, which is also the spec-decoding path the client posts on every update. On a scan source the kernel would materialise the whole column, so the engine swaps in `_streaming_envelope_plan`: one streaming `group_by` collect over the same grid with `min_by`/`max_by` for the x at each extremum (order-independent, so a scan is never sorted-checked — its x dtype is gated off the schema, and its null/NaN contract rides the domain probe as `physical_minmax(contract_cols=...)`, extra outputs of the same select; a zoomed request resolves no domain and so is not checked, but the first request of a figure is unzoomed). A **grouped** line keeps equal-row-count buckets inside each group (a frame sorted by group then x is not globally sorted), and is not sorted-checked either. Preserves extrema and spikes on every path.
...
├── physical_minmax(cols, schema, memoize, contract_cols)  ← per-column unfiltered (min, max); kept on the builder only when memoize=True; `contract_cols` add the line-x null/NaN counts to the same select
├── check_line_x(col)            ← minmax-line x contract: dtype (no collect), then null count, one is_sorted pass, and an O(1) trailing-NaN probe
...
- **Targets** (`get_cube_target_spec(axis_range, schema)`; dim order is part of cube identity):
  ...
  x-width envelopes, but the cube pads its upper bound by `_HIST_BIN_EPSILON` and floors on true
  division while the trace grid comes from `_bucket_grid` (ceiling division on integer x), so their
  bucket edges can differ; the committed delta replaces the live envelope either way.
...
|----------|-------------|
| `LF.py` `check_line_x` | One pass over x to verify the minmax-line x contract — O(n); `assume_sorted` available as opt-in |
