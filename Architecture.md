...
- `_programmaticOp` guard makes the handlers ignore the Plotly events of programmatic Plotly operations (`fvRunProgrammaticPlotlyOp`: axis locks, drag mode, selection box snap-back). Reset and deselect do not hold it: `Plotly.react` emits no relayout, and a guard held for the round trip would drop the user's zooms. `handleSelected` additionally checks `_selectionMatches` + `_selectionBoxMatches` to suppress the spurious `plotly_selected` re-fire that Plotly emits after a `Plotly.react` re-render.
