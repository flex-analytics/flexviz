...
- `_programmaticOps` counts the programmatic Plotly operations in progress (`fvRunProgrammaticPlotlyOp`: axis locks, drag mode, selection box snap-back). While it is above zero, the handlers ignore Plotly events. Reset and deselect do not hold it: `Plotly.react` emits no relayout, and a guard held for the round trip would drop the user's zooms. `handleSelected` additionally checks `_selectionMatches` + `_selectionBoxMatches` to suppress the spurious `plotly_selected` re-fire that Plotly emits after a `Plotly.react` re-render.
