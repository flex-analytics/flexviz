...
- 4 downsampling strategies, selected via `downsample` param. `minmax`, `lttb` and `fpcs` share one **x-width bucket pass**; `nth` stands apart.
  ...
  - **`"nth"`** — uniform stride, no grid and no data check, only the dtype gate. Resident: the `every_nth` Rust kernel (`pl.col(...).filter(vp).flexviz.every_nth(n_points)`), grouped or not — stride computed inside the kernel with no `len()` dependency, so N sub-traces parallelize in one `select()`; grouped = row-count buckets. The kernel materializes the whole column, so scans stream instead. Ungrouped `nth_plan`: two passes (`pl.len()` for stride, then `gather_every(stride).head(n_points)`) because one streaming plan cannot count and gather; unfiltered the count is Parquet metadata, and a viewport is a frame-level filter the reader prunes row groups with. `gather_every` keeps stride phase across morsels (undocumented; `TestNthScanPlan` pins it). Grouped `grouped_nth_plan`: streaming `group_by` for per-group strides, then an ordered `collect_batches` fold with a per-group offset — `.over()` row indices and non-reduction aggs do not stream in Polars 1.44 (`TestGroupedNthScanPlan` pins parity).
