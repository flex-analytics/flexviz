...
class LFQueryBuilder:
    ...
    def physical_minmax(
        ...
        *,
        ...
        unfiltered file. It collects with ``collect_engine``: on a resident
        frame read from Parquet the streaming engine is 5x slower than the
        in-memory one for a filtered min/max over many rows (215 ms vs 40 ms
        at 12.5M surviving rows of 100M, M5). On a single-chunk frame the two
        engines tie.
        """
